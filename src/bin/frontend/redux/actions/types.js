// Reducer types for content
export const CONTENT_HAS_ERROR = "CONTENT_HAS_ERROR";
export const CONTENT_IS_LOADING = "CONTENT_IS_LOADING";
export const CONTENT_FETCH_SUCCESS = "CONTENT_FETCH_SUCCESS";

export const FILEDIR_HAS_ERROR = "FILEDIR_HAS_ERROR";
export const FILEDIR_IS_LOADING = "FILEDIR_IS_LOADING";
export const FILEDIR_FETCH_SUCCESS = "FILEDIR_FETCH_SUCCESS";

export const UPDATE_ERROR = "UPDATE_ERROR";
export const UPDATE_LOADING = "UPDATE_LOADING";
export const UPDATE_SUCCESS = "UPDATE_SUCCESS";

export const NEW_FILE_ERROR = "NEW_FILE_ERROR";
export const NEW_FILE_LOADING = "NEW_FILE_LOADING";
export const NEW_FILE_SUCCESS = "NEW_FILE_SUCCESS";

export const ELEMENT_HAS_ERROR = "ELEMENT_HAS_ERROR";
export const ELEMENT_IS_LOADING = "ELEMENT_IS_LOADING";
export const ELEMENT_FETCH_SUCCESS = "ELEMENT_FETCH_SUCCESS";

export const WORKING_FILE_PATH = "WORKING_FILE_PATH";
export const WORKING_FILE_ELEMENT = "WORKING_FILE_ELEMENT";

// Reducer types for bansa
export const SET_BANSA_FILTER = "SET_BANSA_FILTER";
export const BansaFilter = {
  MARKDOWN_VIEW: "MARKDOWN_VIEW",
  EDITOR_VIEW: "EDITOR_VIEW",
  TREE_VIEW: "TREE_VIEW"
};

export const SHOW_NEW_FILE_MODAL = "SHOW_NEW_FILE_MODAL";
